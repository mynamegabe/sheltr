                            ...
                            <Card className="shadow-none border-0 bg-sidebar-accent/50">
                                <CardContent className="px-2 space-y-4">
                                    ...
                                    <div className="flex justify-center">
                                        ...
                                        >
                                            <ArrowUpDown className="w-3.5 h-3.5 mr-1.5" /> Swap Locations
                                        </Button>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Travel Mode</Label>
                                        <RadioGroup
                                            value={travelMode}
                                            onValueChange={(val: 'WALK' | 'TRANSIT') => setTravelMode(val)}
                                            className="flex gap-2"
                                        >
                                            <div className="flex items-center space-x-2">
                                                <RadioGroupItem value="WALK" id="mode-walk" />
                                                <Label htmlFor="mode-walk" className="flex items-center gap-1.5 cursor-pointer">
                                                    <Footprints className="w-4 h-4" /> Walk
                                                </Label>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                                <RadioGroupItem value="TRANSIT" id="mode-transit" />
                                                <Label htmlFor="mode-transit" className="flex items-center gap-1.5 cursor-pointer">
                                    ...
                                    {travelMode === 'TRANSIT' && (
                                        <div className="space-y-4 pt-2 animate-in fade-in slide-in-from-top-2">
                                            <div className="space-y-2">
                                                <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Allowed Modes</Label>
                                                <div className="grid grid-cols-2 gap-2">
                                                    {['BUS', 'SUBWAY', 'TRAIN', 'LIGHT_RAIL'].map((mode) => (
                                                        <div key={mode} className="flex items-center space-x-2">
                                                            <Checkbox
                                                                ...
                                                                onCheckedChange={(checked) => {
                                                                    if (checked) {
                                                                        setAllowedTransitModes([...allowedTransitModes, mode])
                                                                    } else {
                                                                        setAllowedTransitModes(allowedTransitModes.filter(m => m !== mode))
                                                                    }
                                                                }}
                                                            />
                                                            <Label htmlFor={`mode-${mode}`} className="text-xs cursor-pointer capitalize">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                >
                                                    <div className="flex items-center space-x-2">
                                                        <RadioGroupItem value="LESS_WALKING" id="pref-less-walk" />
                                                        <Label htmlFor="pref-less-walk" className="text-xs cursor-pointer">Less Walking</Label>
                                                    </div>
                                                    <div className="flex items-center space-x-2">
                                                        <RadioGroupItem value="FEWER_TRANSFERS" id="pref-fewer-transfer" />
                                                        <Label htmlFor="pref-fewer-transfer" className="text-xs cursor-pointer">Fewer Transfers</Label>
                                                    </div>
                                                </RadioGroup>
                                            </div>
                                        </div>
                                    ...
                                    >
                                        {loading ? (
                                            <>
                                                <Search className="w-3.5 h-3.5 animate-spin" /> Analyzing...
                                            </>
                                        ) : (
                                            <>
                        ...
                        <>
                            ...
                            <SidebarGroup>
                                <div className="px-2 flex items-center justify-between mb-2">
                                    <SidebarGroupLabel className="p-0">Routes Found ({routes.length})</SidebarGroupLabel>
                                ...
                                <SidebarGroupContent>
                                    <SidebarMenu className="space-y-2 px-2">
                                        {sortedRoutes.map((route, idx) => {
                                            ...
                                            const color = ROUTE_COLORS[originalIndex % ROUTE_COLORS.length];
                    ...
                    <div className="w-full h-full bg-gray-100 dark:bg-neutral-900">
                        ...
                        >
                            ...
                            {routes.map((route, idx) => {
                                ...
                                // If detailed steps are missing, fallback to the main encoded polyline
                                let segments: any[] = []
                                ...
                                if (steps && steps.length > 0) {
                                    ...
                                    segments = [{
                                        positions: decodePolyline(route.data?.polyline?.encodedPolyline || ''),
                                        mode: 'TRANSIT' // Default solid if no breakdown
                                    }]
                                }
                                ...
                                return (
                                    <div key={idx}>
                                        ...
                                        {segments.map((seg, segIdx) => {
                                            ...
                                            // Only show marker if mode changes
                                            ...
                                            let colorClass = '#22c55e' // Default green

                                            if (currMode === 'TRANSIT') { IconComponent = Bus; colorClass = '#3b82f6'; }
                                            else if (currMode === 'SUBWAY' || currMode === 'TRAIN') { IconComponent = Train; colorClass = '#f97316'; }
                                            else if (currMode === 'LIGHT_RAIL' || currMode === 'TRAM') { IconComponent = TramFront; colorClass = '#6366f1'; }
                                            else if (currMode === 'WALK') { IconComponent = Footprints; colorClass = '#10b981'; }
